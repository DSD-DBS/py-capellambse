...
class PhysicalPath(
    ...
    def involved_links(self) -> m.ElementList[PhysicalLink]:
        paths = self.involved_paths.by_class("PhysicalLink", single=False)
        assert all(isinstance(i, PhysicalLink) for i in paths)
        return t.cast(m.ElementList[PhysicalLink], paths)
