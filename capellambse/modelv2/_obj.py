...
_E = t.TypeVar("_E", bound="enum.Enum")
...
class ElementList(cabc.MutableSequence[_O], t.Generic[_O]):
    ...
    def is_coupled(self) -> bool:
        ...
        """
        return isinstance(self.__wrapped, _ModelCoupler)
    ...
    def __add__(self, x, /) -> ElementList[_O]:
        return ElementList([*self.__wrapped, *x.__wrapped])
...
class _ModelCoupler(cabc.MutableSequence[_O], t.Generic[_O]):
...
class AbstractDescriptor(abc.ABC, t.Generic[_T_co]):
    ...
    def __set_name__(self, owner: type[ModelElement], name: str) -> None:
        ...
        if self._resolve_super is not AbstractDescriptor._resolve_super:
            for cls in owner.__mro__[1:]:
                try:
                    parent = getattr(cls, name)
                except AttributeError:
                    continue
                if not isinstance(self, Single) and isinstance(parent, Single):
                    parent = parent._wrapped
                if type(parent) is not type(self):
                    raise RuntimeError(
                        "Super resolution of descriptor properties is"
                        " not supported for different descriptor classes"
                        f" (self: {type(self).__name__},"
                        f" parent: {type(parent).__name__})"
                    )
                self._resolve_super(parent)
                break
            else:
                self._resolve_super(None)

    def _resolve_super(self, parent: AbstractDescriptor | None) -> None:
        pass
...
class Containment(RelationshipDescriptor[_O_co]):
    ...
    def __init__(
        self, tag: str | None, /, *classes: UnresolvedClassName
    ) -> None:
    ...
    def _resolve_super(self, parent: AbstractDescriptor | None) -> None:
        if self._tag is None:
            if parent is None:
                raise TypeError(
                    "Cannot resolve Containment tag from super class:"
                    f" No super class found which defines {self.__name__}"
                )
            if not isinstance(parent, Containment):
                raise TypeError(
                    "Cannot resolve Containment tag from super class:"
                    "Expected another Containment descriptor,"
                    f" found {type(parent).__name__} on {parent._qualname}"
                )
            assert parent._tag is not None
            self._tag = parent._tag
            self._key = parent._key
    ...
    def __get__(self, obj, _) -> te.Self | ElementList[_O_co]:
        ...
        if self._key not in data:
            data[self._key] = ElementList(_ModelCoupler(obj._model))
        assert isinstance(data[self._key], ElementList)
    ...
    def from_xml(
        ...
        if self._tag is None:
            raise RuntimeError(
                "self._tag has not been resolved from super class,"
                f" call __set_name__ on {self!r}"
            )
    ...
    def to_xml(
        ...
        if self._tag is None:
            raise RuntimeError(
                "Tag has not been resolved from super class,"
                f" call __set_name__ on {self!r}"
            )
    ...
    def resolve(self, obj: ModelElement) -> None:
        ...
        data[self._key] = ElementList(_ModelCoupler(obj._model, children))
...
class Association(RelationshipDescriptor[_O_co]):
    ...
    def __init__(
        self, attr: str | None, /, *classes: UnresolvedClassName
    ) -> None:
        if not classes:
            raise TypeError("No valid target class specified")
    ...
    def _resolve_super(self, parent: AbstractDescriptor | None) -> None:
        if self._attr is None:
            if parent is None:
                raise TypeError(
                    "Cannot resolve Association attribute from super class:"
                    f" No super class found which defines {self.__name__}"
                )
            if not isinstance(parent, Association):
                raise TypeError(
                    "Cannot resolve Association attribute from super class:"
                    "Expected another Association descriptor,"
                    f" found {type(parent).__name__} on {parent._qualname}"
                )
            assert parent._attr is not None
            self._attr = parent._attr
            self._key = parent._key
    ...
    def from_xml(
        ...
        if self._attr is None:
            raise RuntimeError(
                "self._attr has not been resolved from super class,"
                f" call __set_name__ on {self!r}"
            )
...
class Allocation(RelationshipDescriptor[_O_co]):
    ...
    def __init__(
        ...
        allocinfo: tuple[str, str] | tuple[str, str, str] | None,
        /,
        *classes: UnresolvedClassName,
    ) -> None:
        ...
        self._alloctype: dict[ClassName, ClassName] | None
        if isinstance(alloctype, cabc.Mapping):
            self._alloctype = {
                _resolve_class_names(k)[0]: _resolve_class_names(v)[0]
                for k, v in alloctype.items()
            }
        elif isinstance(alloctype, tuple):
            (at_resolved,) = _resolve_class_names(alloctype)
            self._alloctype = {
                at_resolved: i for i in _resolve_class_names(*classes)
            }
        else:
            assert alloctype is None
            self._alloctype = None

        if allocinfo is None:
            self._tag = self._linkattr = self._backlink = None
        elif isinstance(allocinfo, tuple) and 2 <= len(allocinfo) <= 3:
            ai: t.Any = allocinfo
            if len(ai) == 2:
    ...
    def _resolve_super(self, parent: AbstractDescriptor | None) -> None:
        if self._alloctype is None:
            if parent is None:
                raise TypeError(
                    "Cannot resolve Allocation information from super class:"
                    f" No super class found which defines {self.__name__}"
                )
            if not isinstance(parent, Allocation):
                raise TypeError(
                    "Cannot resolve Allocation information from super class:"
                    "Expected another Allocation descriptor,"
                    f" found {type(parent).__name__} on {parent._qualname}"
                )

            assert parent._alloctype is not None
            assert parent._tag is not None
            assert parent._linkattr is not None

            self._alloctype = parent._alloctype
            self._tag = parent._tag
            self._linkattr = parent._linkattr
            self._backlink = parent._backlink
            self._key = parent._key
    ...
    def from_xml(
        ...
        if self._linkattr is None:
            raise RuntimeError(
                "self._linkattr has not been resolved from super class,"
                f" call __set_name__ on {self!r}"
            )
    ...
    def resolve(self, obj: ModelElement) -> None:
        assert self._alloctype is not None
        data = obj.__dict__["__modeldata"]
...
class TypeFilter(RelationshipDescriptor[_O_co], t.Generic[_O_co]):
    ...
    def __init__(self, attr: str, cls: UnresolvedClassName) -> None:
        if attr is None:
            raise TypeError("None not allowed as first argument to TypeFilter")
    ...
    def __set_name__(self, owner: type[ModelElement], name: str) -> None:
        ...
        if self._attr_name == name:
            raise TypeError(f"Cannot filter self ({owner.__name__}.{name})")

        self._wrapped = getattr(owner, self._attr_name, None)
        if self._wrapped is None:
            raise TypeError(
                f"Filter target attribute {self._attr_name!r}"
                f" not defined on {owner.__name__}"
                " (make sure the filter is defined after the target)"
            )
