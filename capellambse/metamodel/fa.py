...
class FunctionalChain(
    ...
    control_nodes = m.DeprecatedAccessor["ControlNode"]("sequence_nodes")
...
class AbstractFunction(
    ...
    kind = m.EnumPOD("kind", FunctionKind)  # type: ignore[assignment]
    ...
    realizing_functions = m.Backref["AbstractFunction"](
        (NS, "AbstractFunction"), "realized_functions"
    )
...
class ComponentExchange(
    ...
    allocating_physical_links = m.Backref["cs.PhysicalLink"](
        (ns.CS, "PhysicalLink"), "allocated_functional_exchanges"
    )
    allocating_physical_paths = m.Backref["cs.PhysicalPath"](
        (ns.CS, "PhysicalPath"), "allocated_exchanges"
    )
    ...
    def allocating_physical_link(self) -> cs.PhysicalLink | None:
        warnings.warn(
            (
                ...
                " because it only takes into account the first allocated link."
...
class ComponentPort(
    ...
    kind = m.EnumPOD("kind", ComponentPortKind)  # type: ignore[assignment]
    owner = m.DeprecatedAccessor[m.ModelElement]("parent")
    exchanges = m.Backref["ComponentExchange"](
        (NS, "ComponentExchange"), "source", "target"
    )
    realized_ports = m.Allocation["ComponentPort"](
...
class FunctionalChainInvolvementLink(
    ...
    source = m.Single["FunctionalChainInvolvementFunction"](  # type: ignore[assignment]
        m.Association((NS, "FunctionalChainInvolvementFunction"), "source")
    )
    target = m.Single["FunctionalChainInvolvementFunction"](  # type: ignore[assignment]
    ...
    def __getattr__(attr):
        if attr == "AbstractExchange":
            raise NotImplementedError(
                "AbstractExchange has been removed,"
                " use FunctionalExchange/ComponentExchange directly"
                " or another common superclass"
            )
