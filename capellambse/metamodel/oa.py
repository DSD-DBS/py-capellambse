...
class OperationalAnalysis(cs.BlockArchitecture):
    ...
    activity_package = m.DeprecatedAccessor["OperationalActivityPkg"](
        "activity_pkg"
    )
    ...
    def root_activity(self) -> OperationalActivity:
        ...
        if len(candidates) > 1:
            raise m.BrokenModelError(
                "More than one Activity in the root ActivityPkg"
            )
        return candidates[0]
    ...
    def root_entity(self) -> Entity:
        ...
        if len(candidates) > 1:
            raise m.BrokenModelError(
                "More than one Entity in the root EntityPkg"
            )
        return candidates[0]
...
class Entity(
    ...
    exchanges = m.Alias["CommunicationMean"](  # type: ignore[assignment]
        "communication_means"
    )
...
class CommunicationMean(fa.ComponentExchange):
    ...
    allocated_interactions = m.Alias["fa.FunctionalExchange"](
