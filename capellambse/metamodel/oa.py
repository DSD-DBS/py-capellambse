...
class OperationalAnalysis(cs.BlockArchitecture):
    ...
    def all_actors(self) -> m.ElementList["Entity"]:
        return self.all_entities.by_is_actor(True)
...
class OperationalActivity(fa.AbstractFunction):
    ...
    functional_chains = m.TypeFilter[  # type: ignore[assignment]
        "OperationalProcess"
    ...
    inputs = m.Backref["fa.FunctionalExchange"](
        (ns.FA, "FunctionalExchange"), lookup="target"
    )
    outputs = m.Backref["fa.FunctionalExchange"](
