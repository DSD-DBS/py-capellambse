...
class LogicalArchitecture(cs.ComponentArchitecture):
    ...
    @property
    def actor_exchanges(self) -> m.ElementList[fa.ComponentExchange]:
        all_ex = self._model.search((ns.FA, "ComponentExchange"), below=self)
        incoming = all_ex.by_target(self.all_actors, single=False)
        outgoing = all_ex.by_source(self.all_actors, single=False)
...
class LogicalFunction(fa.AbstractFunction):
    ...
    functions = m.Containment["LogicalFunction"](None, (NS, "LogicalFunction"))
    realized_functions = m.Allocation["sa.SystemFunction"](
        None, None, (ns.SA, "SystemFunction")
    )
    owner = m.Single(
...
class LogicalComponent(
    ...
    realized_components = m.Allocation["sa.SystemComponent"](
        None, None, (ns.SA, "SystemComponent")
    )
