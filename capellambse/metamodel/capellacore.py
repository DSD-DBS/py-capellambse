...
class Involvement(Relationship, abstract=True):
    ...
    target = m.Alias["InvolvedElement"]("involved")
...
class PropertyValueGroup(Namespace):
    ...
    values = m.DirectProxyAccessor(  # TODO not in metamodel?
        m.ModelElement,
        (
            StringPropertyValue,
...
class PropertyValuePkg(Structure):
    ...
    packages = m.DirectProxyAccessor(PropertyValuePkg, aslist=m.ElementList)
    enumeration_property_types = (  # TODO not in metamodel?
        m.DirectProxyAccessor(EnumerationPropertyType, aslist=m.ElementList)
    )
    groups = m.DirectProxyAccessor(  # TODO not in metamodel?
        PropertyValueGroup,
        aslist=m.ElementList,
        mapkey="name",
        mapvalue="values",
