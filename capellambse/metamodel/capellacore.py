...
if t.TYPE_CHECKING:
    from . import information_datavalue, requirement
...
class ReuseLink(Relationship):
    reused = m.Association["ReuseableStructure"](
        "reused", (NS, "ReuseableStructure")
    )
...
class Classifier(GeneralizableElement, abstract=True):
    features = m.Containment["Feature"]("ownedFeatures", (NS, "Feature"))
...
class AbstractExchangeItemPkg(Structure, abstract=True):
    exchange_items = m.Containment["AbstractExchangeItem"](
...
class PropertyValueGroup(Namespace):
    ...
    values = m.Containment["PropertyValue"](
        "ownedPropertyValues",
        (NS, "StringPropertyValue"),
        (NS, "BooleanPropertyValue"),
        (NS, "IntegerPropertyValue"),
        (NS, "FloatPropertyValue"),
        (NS, "EnumerationPropertyValue"),
        # TODO list_extra_args={"mapkey": "name", "mapvalue": "value"},
    )
