...
def load(model: _model.Model, entrypoint: pathlib.PurePosixPath) -> None:
    ...
    referenced_analysis, semantic_resources = _load_aird(
    ...
    for uuid, attr in lazy_attributes:
        ...
        getattr(type(obj), attr).resolve(obj)


def _load_aird(
    resources: cabc.Mapping[str, filehandler.FileHandler],
    resname: _obj.ResourceName,
) -> tuple[etree._Element, list[_obj.ResourceName]]:
    referenced_analysis: list[str] = []
...
def _find_analysis(parser: etree.XMLParser) -> etree._Element:
