...
from capellambse.loader import core
...
class Accessor(t.Generic[U_co], metaclass=abc.ABCMeta):
    ...
    def __set_name__(self, owner: type[t.Any], name: str) -> None:
        ...
        if super_acc is not None and (
            not isinstance(super_acc, Accessor)
            or type(self) is not type(super_acc)
        ):
            raise RuntimeError(
                f"Cannot override a {type(self).__name__!r}"
                f" with a {type(super_acc).__name__!r}"
            )
...
class Alias(Accessor["_obj.ElementList[T_co]"], t.Generic[T_co]):
    ...
    def __set__(
        self, obj: _obj.ModelObject, value: cabc.Iterable[T_co]
    ) -> None:
...
class Relationship(Accessor["_obj.ElementList[T_co]"], t.Generic[T_co]):
    ...
    def _resolve_super_attributes(
        ...
        if super_acc is None:
            return
        for arg in ("mapkey", "mapvalue", "fixed_length"):
            if self.list_extra_args[arg] is None:
                self.list_extra_args[arg] = super_acc.list_extra_args[arg]  # type: ignore[index]
...
class Allocation(Relationship[T_co]):
    ...
    def __set__(
        ...
        if any(isinstance(i, NewObject) for i in value):
            raise TypeError("Cannot create new objects on an Allocation")
        value = t.cast(cabc.Iterable[T_co], value)

        # TODO Remove this extra check when removing deprecated features
        if self.tag is None:
            raise TypeError("Cannot set: XML tag not set")
    ...
    def insert(
        ...
        if isinstance(value, NewObject):
            raise TypeError("Cannot create objects in an Allocation")
    ...
    def _resolve_super_attributes(
        ...
        super()._resolve_super_attributes(super_acc)
        if None not in (self.tag, self.alloc_type, self.attr, self.backattr):
            return
    ...
    def __init__(
        ...
        *,
        ...
        if class_ is None:
            ...
            try:
                ns = class_.__capella_namespace__  # type: ignore[attr-defined]
            except AttributeError:
                raise RuntimeError(
        ...
        for value in values:
            if not isinstance(value, class_):
                raise TypeError(
                    ...
                    f" {self.class_[0].viewpoint}:{self.class_[1]},"
                    f" not {type(value)!r}"
                )
    ...
    def __init__(
        ...
        *attrs: str,
        ...
        if (
            ...
            self.class_ = (class_.__capella_namespace__, class_.__name__)  # type: ignore[attr-defined]
...
class Filter(Accessor["_obj.ElementList[T_co]"], t.Generic[T_co]):
    ...
    def __init__(self, attr: str, class_: _obj.UnresolvedClassName, /) -> None:
        ...
        self.class_ = _obj.resolve_class_name(class_)
        self.list_type = make_coupled_list_type(self)
        self.wrapped: Accessor[_obj.ElementList[T_co]]  # type: ignore
    ...
    def insert(
        ...
        if not isinstance(self.wrapped, Relationship):
            raise AttributeError(f"Cannot insert into {self._qualname}")
...
class Containment(Relationship[T_co]):
    ...
    def insert(
        ...
        if isinstance(value, NewObject):
            ...
            ns = cls.__capella_namespace__  # type: ignore[attr-defined]
            xtype = elmlist._model.qualify_classname((ns, cls.__name__))
    ...
    def _match_xtype(
        ...
        if cls is _obj.ModelElement:
            raise TypeError(f"Cannot insert elements into {self._qualname!r}")
