...
class Allocation(Relationship[T_co]):
    ...
    def __repr__(self) -> str:
        if self.alloc_type is not None:
            alloc_type = f" on {self.alloc_type[0].alias}:{self.alloc_type[1]}"
        else:
            alloc_type = ""
        return (
            f"<{type(self).__name__} {self._qualname!r}"
            f" to {self.class_[0].alias}:{self.class_[1]}"
            f" via {self.tag!r}{alloc_type}>"
        )
...
class Filter(Accessor["_obj.ElementList[T_co]"], t.Generic[T_co]):
    ...
    def __init__(self, attr: str, class_: _obj.UnresolvedClassName, /) -> None:
        ...
        self.class_ = _obj.resolve_class_name(class_)
        self.list_type = make_coupled_list_type(self)
        self.wrapped: Relationship[T_co]  # type: ignore
...
class Containment(Relationship[T_co]):
    ...
    def _guess_xtype(
        ...
        if len(candidates) > 1:
            raise RuntimeError("Multiple eligible classes found: {candidates}")
